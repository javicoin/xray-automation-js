 ...
 * @param {string | undefined} browseBase e.g. https://your-site.atlassian.net/browse/
 */
function issueBrowseLink(key, browseBase) {
    if (!browseBase || typeof browseBase !== 'string') {
        return key;
    }
    const base = browseBase.endsWith('/') ? browseBase : `${browseBase}/`;
    return `${base}${key}`;
}
...
export async function submitCucumberTestResults(reportsDirectory, config, testEvidenceFile = null) {     
    ...
    const jiraBrowseBaseUrl = multipartConfig.jiraBrowseBaseUrl;
    ...
    return xrayClient.submitResultsMultipart(resultsFile, multipartConfig)
        .then( response => { 
            console.log('Test Execution created: \'' + issueBrowseLink(response.key, jiraBrowseBaseUrl) + '\'');
            if (testEvidenceFile)
...
export async function updateTestRunEvidence(testEvidenceFile, options = {}) {       
    const testEvidenceJson = FilesHelper.getJsonContent(testEvidenceFile);
    const jiraBrowseBaseUrl = options.jiraBrowseBaseUrl ?? testEvidenceJson.jiraBrowseBaseUrl;
    ...
    console.log('Browserstack public links added successfully in Test Execution \'' + issueBrowseLink(testEvidenceJson.testExecutionKey, jiraBrowseBaseUrl) + '\'');    
}
